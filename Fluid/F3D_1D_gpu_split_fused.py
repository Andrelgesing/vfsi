...
class F3D(object):
    ...
    def get_p_force(self, omega):
        ...
        if self.n_x_fluid*self.n_y_fluid < 64*65:
            ...
            #print('Likely out of memory during the inversion of the H-matrix')
            #print('Trying collumn-wise')
 
        #else:
            n = self.w.shape[1]
            m = self.w.shape[0]
            block_size = min(4096, m, n)
            #p = cp.empty((n, m), dtype=cp.complex64)
            p_cpu = np.empty((n, m), dtype=np.complex64)
            #density = cp.count_nonzero(a_bem).item() / (a_bem.shape[0] * a_bem.shape[1])
            #print(f"A bem Matrix density: {density:.2%}\n")
            a_bem_cpu = cp.asnumpy(a_bem)
            lu, piv = lu_factor_cpu(a_bem_cpu)
            for j in range(0, m, block_size):
                end_ = min(j + block_size, m)
                rhs_block = self.w[j:end_, :].toarray().get().T  # shape (n, block_size)
                x_block = lu_solve_cpu((lu, piv), rhs_block)
                #rhs_block = cp.asarray(rhs_block, dtype=cp.complex64)
                #x_block = cp.linalg.solve(a_bem, rhs_block)  # shape (n, block_size)
                #print(j)
                p_cpu[:, j:end_] = -self.fluid.mu_f / self.l_c * x_block
            p = cp.asarray(p_cpu)
            #n = self.w.shape[1]
            #m = self.w.shape[0]
            #block_size = min(512, m, n)
            ##p = cp.empty((n, m), dtype=cp.complex64)
            #p_cpu = np.empty((n, m), dtype=np.complex64)
            ##density = cp.count_nonzero(a_bem).item() / (a_bem.shape[0] * a_bem.shape[1])
            ##print(f"A bem Matrix density: {density:.2%}\n")
            #lu, piv = lu_factor_cpu(a_bem)
            #for j in range(0, m, block_size):
            #    end_ = min(j + block_size, m)
            #    rhs_block = self.w[j:end_, :].toarray().T  # shape (n, block_size)
            #    x_block = lu_solve((lu, piv), rhs_block)
            #    #rhs_block = cp.asarray(rhs_block, dtype=cp.complex64)
            #    #x_block = cp.linalg.solve(a_bem, rhs_block)  # shape (n, block_size)
            #    #print(j)
            #    p[:, j:end_] = -self.fluid.mu_f / self.l_c * x_block

            #m = self.w.shape[1]
            #n = self.w.shape[0]
            #p = cp.empty((n, m), dtype=cp.complex64).T  # or cp.float32 if real-valued
            #for j in range(m):
            #    rhs = self.w[j, :].toarray()  # make column dense (shape (n, 1))
            #    #x = sp.linalg.spsolve(a_bem, rhs.ravel())  # solution x = A⁻¹ b
            #    x = cp.linalg.solve(a_bem, rhs.ravel())
            #    p[:, j] = -self.fluid.mu_f/self.l_c * x
        if self.print_time:
            tt = time.time() - t_init
