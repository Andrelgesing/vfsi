...
class F3D(object):
    ...
    def get_p_force(self, omega):
        ...
        if type(self.a_bem) == cp.ndarray:
            ...
            #a_bem_csr = sp.csr_matrix(self.a_bem, dtype=complex)
            #p = -self.fluid.mu_f/self.l_c*sp.linalg.spsolve(self.a_bem , self.w.T)
            m = self.w.shape[1]
            n = self.w.shape[0]

            p_result = cp.empty((n, m), dtype=cp.complex64)  # or cp.float32 if real-valued

            for j in range(m):
                rhs = self.w[j, :].toarray()  # make column dense (shape (n, 1))
                x = sp.linalg.spsolve(self.a_bem, rhs.ravel())  # solution x = A⁻¹ b
                p_result[j, :] = -self.fluid.mu_f/self.l_c * x
            p = p_result.T
        #del a_inv
        if self.print_time:
            tt = time.time() - t_init
        ...
        #p *= self.wx[:, None] * self.wy[None, :]
        p *= self.weight_int[:, None]
        #if p.dtype != cp.complex64:
        ...
        if self.print_time:
            tt = time.time() - t_init
            print('Multiplications took %f seconds\n' % (tt))
