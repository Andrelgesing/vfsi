...
np.save('Data/tt_2d_cpu.npy', data)  
...
data = np.array([frequency/1e3, 1e9*np.abs(phi_2d)])
np.save('Data/phi_2d.npy', data)

data = np.array([frequency/1e3, 1e9*np.abs(w_sader[:, -1])])
np.save('Data/phi_sader.npy', data)
