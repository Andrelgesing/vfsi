...
np.save('Data/tt_3d_1d_uniform_cpu.npy', data)

data = np.array([frequency/1e3, 1e9*np.abs(phi_3d)])
np.save('Data/phi_3d_1d_uniform_cpu.npy', data)
...
tt = (time.time() - tt)
data = np.array([n_x_fluid, n_y_fluid, n_f, tt])
np.save('Data/tt_3d_1d_nonuniform_cpu.npy', data)
data = np.array([frequency/1e3, 1e9*np.abs(phi_non_uni)])
np.save('Data/phi_3d_1d_nonuniform_cpu.npy', data)
