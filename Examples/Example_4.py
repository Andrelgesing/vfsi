...
L = cp.asarray(plate.get_linear_form())
#F = cp.asarray(L.get_local())
#F = L.astype(cp.float64)
...
frequency_np = frequency.get()
tt = time.time()
for ff in tqdm(range(n_f)):
    omega = 2*np.pi*frequency_np[ff]
    Fl_3d = vfi.get_p_force(omega)
    A_3d = K_csr - omega**2*M_csr + 1j*omega*Fl_3d
    #A_dense = (A_3d.toarray()).astype(cp.complex64)
    phi = spsolve(A_3d, L)
    #phi = cp.linalg.solve(A_dense, F)
    phi_3d[ff] = plate.get_point_value(phi.get())

tt = (time.time() - tt)
data = np.array([n_x_fluid, n_y_fluid, n_f, tt])
np.save('Data/tt_3d_uniform_gpu.npy', data)   

data = np.array([frequency.get()/1e3, 1e9*np.abs(phi_3d.get())])
np.save('Data/phi_3d_uniform_gpu.npy', data)
...
tt = (time.time() - tt)
data = np.array([n_x_fluid, n_y_fluid, n_f, tt])
np.save('Data/tt_3d_nonuniform_gpu.npy', data)
data = np.array([frequency.get()/1e3, 1e9*np.abs(phi_non_uni.get())])
np.save('Data/phi_3d_nonuniform_gpu.npy', data)
...
probe.fluid = fluid
probe.mat = mat
probe.frequency = frequency.get()
probe.N_modes = 10
probe.n_x = 200
#w = probe.thermal_displacement()
#q_sader = probe.q_factor()
w_sader = probe.get_displacement_per_force().T*w_c

plt.figure(1, figsize=(9, 4))
plt.loglog(frequency.get()/1e3, 1e9*np.abs(phi_3d.get()), linewidth=2.4, label='3D')
plt.loglog(frequency.get()/1e3, 1e9*np.abs(w_sader[:, -1]), ':', linewidth=3,  label='Sader')
